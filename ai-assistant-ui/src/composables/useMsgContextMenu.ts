...
interface MsgCtxMenuState {
  show: boolean; x: number; y: number; index: number; selectionText: string
  pointerX: number; pointerY: number; anchorTop: number
  selLeft: number; selRight: number; selTop: number; selBottom: number
}

interface InlineTransPopState {
  show: boolean; x: number; y: number; text: string; loading: boolean; error: string
  pointerX: number; pointerY: number; anchorTop: number
  selLeft: number; selRight: number; selTop: number; selBottom: number
}
...
export function useMsgContextMenu(deps: MsgCtxDeps) {
  const msgCtxMenu = ref<MsgCtxMenuState>({
    show: false, x: 0, y: 0, index: -1, selectionText: '',
    pointerX: 0, pointerY: 0, anchorTop: 0,
    selLeft: 0, selRight: 0, selTop: 0, selBottom: 0,
  })

  const inlineTranslatePopover = ref<InlineTransPopState>({
    show: false, x: 0, y: 0, text: '', loading: false, error: '',
    pointerX: 0, pointerY: 0, anchorTop: 0,
    selLeft: 0, selRight: 0, selTop: 0, selBottom: 0,
  })
  ...
  function onBubbleContextMenu(e: MouseEvent, globalIndex: number, role: string) {
    ...
    if (x < pad) x = pad
    if (y < pad) y = pad
    let ptrX = e.clientX, ptrY = e.clientY, anchorTop = e.clientY
    let selLeft = 0, selRight = 0, selTop = 0, selBottom = 0
    if (selectionText) {
      const sel = window.getSelection()
      if (sel && sel.rangeCount > 0 && !sel.isCollapsed) {
        ...
        if (bubble.contains(range.commonAncestorContainer)) {
          ...
          if ((rr.width > 0 || rr.height > 0) && rr.bottom >= rr.top) {
            selLeft = rr.left; selRight = rr.right; selTop = rr.top; selBottom = rr.bottom
            ptrX = rr.right + selGap; ptrY = rr.top; anchorTop = rr.top
          }
        }
      }
    }
    msgCtxMenu.value = { show: true, x, y, index: globalIndex, selectionText,
      pointerX: ptrX, pointerY: ptrY, anchorTop, selLeft, selRight, selTop, selBottom }
  }
  ...
  async function copyAssistantSelection() {
    ...
    catch { deps.reportError('copy', 'clipboard unavailable') }
  }
  ...
  function positionInlineTranslatePopoverNearPointer() {
    ...
    const sl = pop.selLeft, sr = pop.selRight, st = pop.selTop, sb = pop.selBottom
    const hasSel = sr > sl + 2 && sb >= st
    let left: number, top: number
    if (hasSel) {
      const placeRight = sr + gap, placeLeft = sl - pw - gap
      ...
      left = pop.pointerX; top = pop.pointerY
  ...
  function closeInlineTranslatePopover() {
    ...
    detachInlinePopLayoutListeners()
    if (positionInlinePopRaf) { cancelAnimationFrame(positionInlinePopRaf); positionInlinePopRaf = 0 }
    inlineTranslatePopover.value = {
      show: false, x: 0, y: 0, text: '', loading: false, error: '',
      pointerX: 0, pointerY: 0, anchorTop: 0, selLeft: 0, selRight: 0, selTop: 0, selBottom: 0,
    }
  }

  async function translateAssistantSelection() {
    const text = msgCtxMenu.value.selectionText
    if (!text || !deps.baseUrl) { closeMsgCtxMenu(); return }
    const { pointerX, pointerY, anchorTop, selLeft, selRight, selTop, selBottom } = msgCtxMenu.value
    closeMsgCtxMenu()
    inlineTranslateAbortCtrl?.abort()
    inlineTranslateAbortCtrl = new AbortController()
    const signal = inlineTranslateAbortCtrl.signal
    inlineTranslatePopover.value = {
      show: true, x: 0, y: 0, text: '', loading: true, error: '',
      pointerX, pointerY, anchorTop, selLeft, selRight, selTop, selBottom,
    }
    await nextTick()
    schedulePositionInlineTranslatePopover()
    attachInlinePopLayoutListeners()
    try {
      const payload = { action: 'translate' as const, text, targetLang: inferInlineTranslateTargetLang(text, deps.targetLang.value) }
      let acc = ''
      let textFlushRaf = 0
      const stream = streamChat(deps.baseUrl, payload, deps.accessToken, signal)
      try {
        for await (const chunk of stream) {
          if (signal.aborted) { inlineTranslatePopover.value.loading = false; return }
          acc += chunk
          if (!textFlushRaf) {
            textFlushRaf = requestAnimationFrame(() => {
              textFlushRaf = 0
              if (!signal.aborted) { inlineTranslatePopover.value.text = acc; schedulePositionInlineTranslatePopover() }
            })
          }
        }
      } finally { if (textFlushRaf) { cancelAnimationFrame(textFlushRaf); textFlushRaf = 0 } }
      if (!signal.aborted) { inlineTranslatePopover.value.text = acc; schedulePositionInlineTranslatePopover() }
    } catch (e: unknown) {
      if (!signal.aborted) { inlineTranslatePopover.value.error = (e instanceof Error ? e.message : String(e)) || 'Translation failed' }
    } finally {
