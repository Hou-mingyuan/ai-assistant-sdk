...
export function usePanelDrag(
  ...
  let pendingDx = 0, pendingDy = 0, flushRaf = 0
  let tentative: { pointerId: number; startX: number; startY: number; headerEl: HTMLElement } | null = null
  ...
  function onTentativeMove(e: PointerEvent) {
    ...
    if (Math.hypot(e.clientX - tentative.startX, e.clientY - tentative.startY) < DRAG_CLICK_PX) return
    const sel = window.getSelection()
    if (sel && sel.rangeCount > 0 && !sel.getRangeAt(0).collapsed) { cleanupTentative(); return }
