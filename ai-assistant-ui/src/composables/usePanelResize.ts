...
export function computeRectFromResizePointer(
  ...
  let wl = wl0, wt = wt0, w = w0, h = h0
  switch (edge) {
    case 'se': w = ex - wl0; h = ey - wt0; break
    case 'sw': w = wl0 + w0 - ex; h = ey - wt0; wl = ex; break
    case 'ne': w = ex - wl0; h = wt0 + h0 - ey; wt = ey; break
    case 'nw': w = wl0 + w0 - ex; h = wt0 + h0 - ey; wl = ex; wt = ey; break
    case 'e': w = ex - wl0; break
    case 'w': w = wl0 + w0 - ex; wl = ex; break
    case 's': h = ey - wt0; break
    case 'n': h = wt0 + h0 - ey; wt = ey; break
  }
  const cl = clamp(w, h)
  w = cl.w; h = cl.h
  switch (edge) {
    case 'se': wl = wl0; wt = wt0; break
    case 'sw': wl = wl0 + w0 - w; wt = wt0; break
    case 'ne': wl = wl0; wt = wt0 + h0 - h; break
    case 'nw': wl = wl0 + w0 - w; wt = wt0 + h0 - h; break
    case 'e': wl = wl0; wt = wt0; break
    case 'w': wl = wl0 + w0 - w; wt = wt0; break
    case 's': wl = wl0; wt = wt0; break
    case 'n': wl = wl0; wt = wt0 + h0 - h; break
  }
  return { wl, wt, w, h }
}
...
export function usePanelResize(
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    const r = computeRectFromResizePointer(resizeDrag.edge, resizeDrag.r0, e.clientX, e.clientY, clampPanelSize)
    pendingRect = r
