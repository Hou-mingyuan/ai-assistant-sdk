 ...
 *
 * <p>Client sends JSON matching {@link ChatRequest}; server streams delta chunks as individual text
 * frames, then sends {@code [DONE]} and closes normally.
 */
public class AiAssistantWebSocketHandler extends TextWebSocketHandler {
                ...
                heartbeatScheduler.scheduleAtFixedRate(
                        () -> {
                            try {
                                ...
                                log.debug(
                                        "Heartbeat ping failed for session {}: {}",
                                        session.getId(),
                                        e.getMessage());
                            }
                        },
    ...
    protected void handleTextMessage(WebSocketSession rawSession, TextMessage message)
        ...
        int maxChars =
                properties.getChatMaxTotalChars() > 0
                        ? properties.getChatMaxTotalChars()
                        : WS_MAX_TEXT_CHARS;
        if (text.length() > maxChars) {
            session.sendMessage(
                    new TextMessage("{\"error\":\"text exceeds " + maxChars + " characters\"}"));
            return;
        }
        ...
        if (historyNode != null && historyNode.isArray()) {
            if (historyNode.size() > 500) {
                session.sendMessage(
                        new TextMessage("{\"error\":\"history exceeds 500 messages\"}"));
                return;
            }
            ...
            var flux =
                    switch (action) {
                        case "translate" -> {
                            String lang =
                                    json.has("targetLang")
                                            ? json.get("targetLang").asText("zh")
                                            : "zh";
                            yield llmService.translateStream(text, lang);
                        }
                        case "summarize" -> llmService.summarizeStream(text);
                        default ->
                                llmService.chatStream(text, hist, systemPrompt, model, imageData);
                    };
