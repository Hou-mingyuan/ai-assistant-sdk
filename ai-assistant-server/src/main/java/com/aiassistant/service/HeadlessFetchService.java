...
/**
 * Uses Playwright headless Chromium to render JS-heavy pages (e.g. Taobao, Tmall) and extract
 * text/images that plain HTTP cannot reach.
 */
public class HeadlessFetchService implements HeadlessFetcher {
    ...
    /**
     * 在独立线程中预装 Chromium 并启动浏览器，避免首个用户请求阻塞 HTTP 线程数分钟。 应在 {@link
     * org.springframework.boot.context.event.ApplicationReadyEvent} 后调用。
     */
        ...
        Thread t =
                new Thread(
                        () -> {
                            try {
                                log.info(
                                        "Headless warmup: downloading Chromium / launching browser in background (may take several minutes on first run)...");
                                ensureBrowser();
    ...
    private Result doFetch(String url) {
        try {
            if (url == null || !(url.startsWith("http://") || url.startsWith("https://"))) {
                log.warn(
                        "Headless fetch rejected: only http/https allowed, got: {}",
            ...
            int timeout =
                    (int)
                            Math.min(
                                    (long) properties.getHeadlessFetchTimeoutSeconds() * 1000,
                                    Integer.MAX_VALUE);
            BrowserContext ctx =
                    browser.newContext(
        ...
        ctx.route(
                ...
                route -> {
                    ...
                    try {
                        ...
                        log.debug(
                                "Headless request blocked by URL safety: {} ({})",
                                requestUrl,
                                e.getMessage());
                        route.abort();
                    }
        ...
        synchronized (lock) {
            ...
            if (!browserInstalled) {
                ...
                try {
                    ...
                    log.warn(
                            "Chromium auto-install failed (may already be installed): {}",
                            e.getMessage());
                }
            }
            if (playwright == null) {
