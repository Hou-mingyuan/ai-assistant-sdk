...
class AgentExecutorTest {
    ...
    void execute_singleStep_success() {
        var trace =
                executor.execute(List.of(new AgentExecutor.AgentStep("Add numbers", "add", "{}")));
    ...
    void execute_stepWithoutTool_usesDescription() {
        var trace =
                executor.execute(
                        List.of(
                                new AgentExecutor.AgentStep(
                                        "Think about the problem", null, null)));
        assertTrue(trace.allSucceeded());
    ...
    void executeTool_unknownTool_throws() {
        assertThrows(
                IllegalArgumentException.class, () -> executor.executeTool("nonexistent", "{}"));
    }
    ...
    void trace_totalElapsedMs_isPositive() {
        var trace = executor.execute(List.of(new AgentExecutor.AgentStep("Add", "add", "{}")));
